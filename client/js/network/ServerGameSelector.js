define( [ 'view/managers/QueryStringManager', 'lib/jsclass/core'], function( QueryStringManager ) {
	return new JS.Class({
		...
		initAndConnetToWebSocket: function() {
			...
			this.connection = new WebSocket( 'ws://' + this.config.HOST + ':' + this.config.PORT );
			console.log('(ServerGameSelector) Connecting to ws://' + this.config.HOST + ':' + this.config.PORT);
		...
		onConnectionOpened: function() {
			...
			var newData = { 'desiredPort': QueryStringManager.getQueryString('game') || this.config.PORT };
