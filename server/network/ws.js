...
define(['http', 'net', 'crypto', 'lib/hashlist'], function( http, net, crypto, HashList ) {
	// WebSocekt Server ------------------------------------------------------------
	// -----------------------------------------------------------------------------
	...
	Server.prototype = {
		...
		remove: function(conn) {
			if (this.connections.remove(conn)) {
				console.log("(WS).Server - removing connection...");
				this.onClose(conn);
			}
		},
		...
		{
			if(this.isOpen === true) {
				console.log("(WS).Server - Already open!");
			};
